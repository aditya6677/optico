				...
				while ( have_posts() ) : the_post();
					...
					switch( get_post_type() ){
						...
						case 'post':  // post
							...
							get_template_part( 'theme-parts/blogbox/blogbox', 'classic' );
							echo '</div>';
							break;

						case 'page':  // page
							echo '<li><i class="ts-optico-icon-angle-right"></i> &nbsp; <span class="ts-list-li-content"><a href="' . get_the_permalink() . '">' . get_the_title() . '</a></span></li>';
							break;

						case 'ts-portfolio':  // Portfolio
							$view = ( !empty($optico_theme_options['pfcat_view']) ) ? $optico_theme_options['pfcat_view'] : 'style-1' ;
						...
						case 'ts-team-member':  // Portfolio
							$view = ( !empty($optico_theme_options['teamcat_view']) ) ? $optico_theme_options['teamcat_view'] : 'style-1' ;
