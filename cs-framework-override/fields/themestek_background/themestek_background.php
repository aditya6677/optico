...
class CSFramework_Option_themestek_background extends CSFramework_Options {
  ...
  public function output() {
    ...
    $button_title   = ( isset( $button_title ) ) ? $button_title : esc_attr__( 'Upload', 'optico' );
    $frame_title    = ( isset( $frame_title  ) ) ? $frame_title  : esc_attr__( 'Upload', 'optico' );
    $insert_title   = ( isset( $insert_title ) ) ? $insert_title : esc_attr__( 'Use Image', 'optico' );
	...
	$btntext_add    = esc_attr__('Add image','optico');
	$btntext_change = esc_attr__('Change image','optico');
	$btntext        = ( empty( $value ) )  ? esc_attr__('Add image','optico') : esc_attr__('Change image','optico');
	...
	echo '<span class="ts-cs-background-text-add-image" style="display:none;">'.esc_attr__('Add image','optico').'</span>';
	echo '<span class="ts-cs-background-text-change-image" style="display:none;">'.esc_attr__('Change image','optico').'</span>';
	...
	echo '<div class="cs-image-preview"><div class="cs-preview"><div class="cs-preview-inner">
			...
			<img src="'. esc_url($preview) .'" alt="' . esc_attr__('Preview','optico') . '" class="'. esc_attr($hidden) .'" />
			<div class="ts-cs-background-heading-noimg'. esc_attr($hidden_text) .'"> ' . esc_attr__('No image selected for background','optico') . ' </div>
		</div></div></div>';

    echo '<a href="javascript:void(0)" class="button button-primary cs-add">'. ( ( empty( $value ) ) ? esc_attr__('Add image','optico') : esc_attr__('Change image','optico') ) .'</a>';
	...
	echo '<small>'. esc_attr__('BG image position','optico') .'</small>';
    echo cs_add_element( array(
        'pseudo'          => true,
        'type'            => 'select',
	...
	echo '<small>'. esc_attr__('BG image attachment','optico') .'</small>';
    echo cs_add_element( array(
        'pseudo'          => true,
        'type'            => 'select',
	...
	echo '<small>'. esc_attr__('BG image size','optico') .'</small>';
    echo cs_add_element( array(
        'pseudo'          => true,
        'type'            => 'select',
	...
	if( isset($this->field['color']) && $this->field['color']==false ){
		...
		echo '<small>'. esc_attr__('BG image color','optico') .'</small>';
		echo cs_add_element( array(
			'pseudo'          => true,
