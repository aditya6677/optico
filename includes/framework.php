 ...
 */
if( ! function_exists( 'optico_cs_framework_init' ) ) {
function optico_cs_framework_init(){
	// Codestar framework config
	defined('CS_OPTION'          ) or define('CS_OPTION',           'optico_theme_options' );
...
add_action( 'init', 'optico_cs_framework_init', 1 );
add_action( 'admin_init', 'optico_cs_framework_init', 1 );
			...
			if( isset($section['fields']) && is_array($section['fields']) ){
				foreach( $section['fields'] as $field ){
					if( isset($field['id']) && isset($field['default']) ){
						...
						$optico_theme_options[$field_id] = $field_val;
					}
				}
				...
				foreach( $sections['sections'] as $section ){
					...
					foreach( $fields as $field ){
						if( isset($field['id']) && isset($field['default']) ){
							$field_id              = $field['id'];
							$field_val             = $field['default'];
							$optico_theme_options['sections']['fields'][$field_id] = $field_val;
						}
					}
				}
			}
...
function themestek_theme_gfonts() {
	...
	// Latin extended
	if( themestek_get_option('typo_latin_extended')==true ){
