			...
			foreach( ${$post_type.'_social_share_services'} as $social ){
				switch($social){
					...
					case 'digg':
						...
						break;
					case 'whatsapp':
						$link = 'https://wa.me/?text='. get_permalink();
						break;
				} // switch end here
...
function themestek_pagination( $wp_query_data=false ){
	...
	$paginateLinks = paginate_links( array(
		...
		'type'      => 'array',
		'prev_text' => '<i class="ts-optico-icon-arrow-left"></i> <span class="ts-hide ts-pagination-text ts-pagination-text-prev">' . esc_attr__( 'Previous page', 'optico' ) . '</span>',
		'next_text' => '<span class="ts-hide ts-pagination-text ts-pagination-text-next">' . esc_attr__( 'Next page', 'optico' ) . '</span> <i class="ts-optico-icon-arrow-right"></i>',
	) );
...
function themestek_get_option( $option, $inner_option='' ){
	...
	if( !is_array($optico_theme_options) ){
		if( is_multisite() ){
			$optico_theme_options = get_site_option('optico_theme_options');
		} else {
			$optico_theme_options = get_option('optico_theme_options');
		}
	}
	$return = '';
	if( isset($optico_theme_options[$option]) ){
		$return = $optico_theme_options[$option];
	} else {
...
function themestek_get_all_registered_sidebars(){
	...
	$return = array( '' => esc_attr__('Default', 'optico') );
...
/**
 * Custom template tags for Optico
 *
 ...
 * @since Optico 1.0
 */
if ( ! function_exists( 'optico_comment_nav' ) ) :
/**
 * Display navigation to next/previous comments when applicable.
 *
 * @since Optico 1.0
 */
function optico_comment_nav() {
	// Are there comments to navigate through?
	if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) :
	?>
	<nav class="navigation comment-navigation">
		<h2 class="screen-reader-text"><?php esc_attr_e( 'Comment navigation', 'optico' ); ?></h2>
		<div class="nav-links">
			<?php
				if ( $prev_link = get_previous_comments_link( esc_attr__( 'Older Comments', 'optico' ) ) ) :
					printf( '<div class="nav-previous">%s</div>', $prev_link );
				endif;
				if ( $next_link = get_next_comments_link( esc_attr__( 'Newer Comments', 'optico' ) ) ) :
...
if ( ! function_exists( 'optico_entry_meta' ) ) :
/**
 * Prints html with meta information for the categories, tags.
 *
 * @since Optico 1.0
 */
function optico_entry_meta( $metafor="blogbox" ) {
			...
			switch( $meta_id ){
				case 'date':
					...
					if ( in_array( get_post_type(), array( 'post', 'attachment' ) ) ) {
						...
						$return .= sprintf( '<span class="ts-meta-line posted-on"><i class="ts-optico-icon-clock"></i> <span class="screen-reader-text ts-hide">%1$s </span><a href="%2$s" rel="bookmark">%3$s</a></span>',
							esc_attr_x( 'Posted on', 'Used before publish date.', 'optico' ),
				...
				case 'author':
					if ( 'post' === get_post_type() ) {
						...
						$return .= sprintf( '<span class="ts-meta-line byline"><i class="ts-optico-icon-user"></i> <span class="author vcard"><span class="screen-reader-text ts-hide">%1$s </span>%2$s</span></span>',
							esc_attr_x( 'Author', 'Used before post author name.', 'optico' ),
							$author
						);
					}
					break;
				case 'comment':
					if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
						$return .= '<span class="ts-meta-line comments-link"><i class="ts-optico-icon-comments-smiley"></i> ';
						ob_start();
						comments_popup_link( esc_attr__( 'Leave a comment', 'optico' ) );
				...
				case 'cat':
					...
					if ( !empty($categories_list) ) {
						...
						$return .= sprintf( '<span class="ts-meta-line cat-links"><i class="ts-optico-icon-category"></i> <span class="screen-reader-text ts-hide">%1$s </span>%2$s</span>',
							esc_attr_x( 'Categories', 'Used before category names.', 'optico' ),
							$categories_list
						);
					}
					break;
				case 'tag':
					$tags_list = get_the_tag_list( '', _x( ', ', 'Used between list items, there is a space after the comma.', 'optico' ) );
					if ( !empty($tags_list) ) {
						if( $tag_link!=true ){
							$tags_list = strip_tags($tags_list);
						}
						$return .= sprintf( '<span class="ts-meta-line tags-links"><i class="ts-optico-icon-tag"></i> <span class="screen-reader-text ts-hide" ts-hide>%1$s </span>%2$s</span>',
							esc_attr_x( 'Tags', 'Used before tag names.', 'optico' ),
							$tags_list
						);
					}
					break;
 ...
 */
if( ! function_exists('optico_categorized_blog') ){
function optico_categorized_blog() {
	if ( false === ( $all_the_cool_cats = get_transient( 'optico_categories' ) ) ) {
		...
		set_transient( 'optico_categories', $all_the_cool_cats );
	}
	if ( $all_the_cool_cats > 1 ) {
		// This blog has more than 1 category so optico_categorized_blog should return true.
		return true;
	} else {
		// This blog has only 1 category so optico_categorized_blog should return false.
		return false;
	}
}
}
...
function themestek_body_start_code(){
	...
	if( !empty($loaderimg) && $preloader_show==true ){
		if( $loaderimg=='custom' && !empty($loaderimage_custom) ){
			...
			if( !empty($imgdata[0]) ){
				$page_loader = themestek_wp_kses('<div class="ts-pre-loader-container ts-pre-loader-custom"><img src="'. esc_url($imgdata[0]).'" alt="'.esc_attr__('Loading...', 'optico').'" /></div>');
			}
		} else {
			$page_loader = '<div class="ts-pre-loader-container">';
			$page_loader .= '<img src="' . get_template_directory_uri() . '/images/loader'. esc_attr($loaderimg).'.svg" alt="'.esc_attr__('Loading...', 'optico').'" />';
...
function themestek_blogbox_comment_count( $echo = false ) {
	...
	if( comments_open() ){
		...
		$return  .= '<div class="themestek-blogbox-comment"><i class="ts-optico-icon-comment"></i> '. $comments .'</div>';
	}
	return $return;
}
}
		...
		if( !empty($return) ){
			switch( $type ){
				case 'position':
					...
					break;
				case 'phone':
					$return = '<div class="ts-team-list-title ts-skincolor"><i class="ts-optico-icon-phone"></i> '. esc_attr__('Phone','optico') .'</div>
						<div class="ts-team-list-value"><a href="tel:' . $return . '">' . $return . '</a></div>';
					break;
				case 'email':
					$return = '<div class="ts-team-list-title ts-skincolor"><i class="ts-optico-icon-mail"></i> '. esc_attr__('Email','optico') .'</div>
				...
				case 'website':
					...
					$return = '<div class="ts-team-list-title ts-skincolor"><i class="ts-optico-icon-world"></i> '. esc_attr__('Website','optico') .'</div>
...
function themestek_create_icon_from_data( $data, $i_tag_only=false ){
	...
	if( isset($data['library']) && isset($data['library_fontawesome']) && isset($data['library_linecons']) && isset($data['library_themify']) ){
		...
		$library_ts_optico = ( !empty($data['library_ts_optico']) ) ? $data['library_ts_optico'] : '';
		...
		if( !empty(${'library_'.$library}) && ${'library_'.$library}!='empty' ){
			$return = do_shortcode('[ts-icon type="'.$library.'" icon_ts_optico="'.$library_ts_optico.'" icon_fontawesome="'.$library_fontawesome.'" icon_linecons="'.$library_linecons.'" icon_themify="'.$library_themify.'"]');
			if( $i_tag_only==true ){
...
function themestek_portfolio_next_prev_btn() {
	...
	if( is_singular('ts-portfolio') ){
		$return = get_the_post_navigation( array(
			'next_text' => '<span class="meta-nav" aria-hidden="true">' . esc_attr__( 'Next', 'optico' ) . '</span> ' .
				'<span class="screen-reader-text ts-hide">' . esc_attr__( 'Next post:', 'optico' ) . '</span> ' .
				'<span class="post-title ts-hide">%title</span>',
			'prev_text' => '<span class="meta-nav" aria-hidden="true">' . esc_attr__( 'Previous', 'optico' ) . '</span> ' .
				'<span class="screen-reader-text ts-hide">' . esc_attr__( 'Previous post:', 'optico' ) . '</span> ' .
				'<span class="post-title ts-hide">%title</span>',
		) );
	}
	return $return;
...
function themestek_portfolio_detailsbox( $type='list'){ // 'list' or 'box'
	...
	if( !empty($box_title) ){
		$box_title = '<h2 class="themestek-pf-detailbox-title">'.$box_title.'</h2>';
	}
	...
	if( isset($pf_details_line) && is_array($pf_details_line) && count($pf_details_line)>0 ){
		foreach( $pf_details_line as $key=>$val ){
			...
			if( !empty($val['pf_details_line_title']) ){ $row_title = sprintf( esc_attr__('%s ', 'optico'), $val['pf_details_line_title'] ); }
...
/**
 *  Blogbox class : themestek_portfoliobox_class()
 */
if ( !function_exists( 'themestek_portfoliobox_class' ) ){
function themestek_portfoliobox_class(){
	$return = '';
	return $return;
}
}
if ( !function_exists( 'themestek_portfoliobox_media_link' ) ){
function themestek_portfoliobox_media_link( $default_icon='ts-optico-icon-search-1', $video_icon='ts-optico-icon-video', $audio_icon='ts-optico-icon-music-alt', $slider_icon='ts-optico-icon-gallery-1' ){
	$icon_code='<i class="ts-optico-icon-search-1"></i>';
...
function themestek_gallerystyle_portfoliobox_media_link( $default_icon='ts-optico-icon-search-1', $video_icon='ts-optico-icon-video', $audio_icon='ts-optico-icon-music-alt', $slider_icon='ts-optico-icon-gallery-1' ){
	$icon_code='<i class="ts-optico-icon-search-1"></i>';
	if( !empty($default_icon) ){
		$icon_code='<i class="' . $default_icon . '"></i>';
	}
...
function themestek_get_post_format_icon( $post_id='' ){
	...
	if( $post_type=='post' ){
		...
		$return = '<i class="ts-optico-icon-' . $icon_class . '"></i>';
	}
	if( !empty($return) ){
...
function themestek_get_boxes( $cpt='blog', $vars=array() ){
	...
	$allword  = (!empty($vars['allword'])) ? $vars['allword'] : esc_attr__('All', 'optico');
	...
	$ts_readmore_link   = (!empty($vars['readmore'])) ? $vars['readmore'] : '';
	if( empty($posts) ){
		global $posts;
	}
...
function themestek_global_blog_template_list( $for_vc=false ){
	$return = array(
		"top-image"       => esc_attr__("Top image and bottom content (default)", "optico"),
		"left-image"      => esc_attr__('Left image and right content ', "optico"),
		"right-image"     => esc_attr__('Right image and left content ', "optico"),
	);
	...
	if( !empty($for) ){
		switch( $for ){
			...
			case 'blog':
				$return = array(
					array(
						'label'	=> esc_attr('Blog Box - Style 1','optico'),
						'value'	=> 'style-1',
						'thumb'	=> get_template_directory_uri() . '/includes/images/blogbox-style-1.jpg',
					),
					array(
						'label'	=> esc_attr('Blog Box - Style 2','optico'),
						'value'	=> 'style-2',
			...
			case 'fidbox':
				$return = array(
					array(
						'label'	=> esc_attr('Facts in Digits Box - Style 1','optico'),
						'value'	=> 'style-1',
						'thumb'	=> get_template_directory_uri() . '/includes/images/fidbox-style-1.jpg',
					),
					array(
						'label'	=> esc_attr('Facts in Digits Box - Style 2','optico'),
						'value'	=> 'style-2',
						'thumb'	=> get_template_directory_uri() . '/includes/images/fidbox-style-2.jpg',
					),
					array(
						'label'	=> esc_attr('Facts in Digits Box - Style 3','optico'),
						'value'	=> 'style-3',
			...
			case 'pricingtable':
				$return = array(
					array(
						'label'	=> esc_attr('Pricing Table - Style 1','optico'),
						'value'	=> 'style-1',
			...
			case 'clients':
				$return = array(
					array(
						'label'	=> esc_attr('Clients Box - Style 1','optico'),
						'value'	=> 'style-1',
						'thumb'	=> get_template_directory_uri() . '/includes/images/clientbox-style-1.jpg',
					),
					array(
						'label'	=> esc_attr('Clients Box - Style 2','optico'),
						'value'	=> 'style-2',
...
function themestek_global_client_template_list( $for_vc=false ){
	$return = array(
		'simple-logo'    => esc_attr__('Simple Logo view', 'optico'),
		'separator-logo' => esc_attr__('Logo with Separator view', 'optico'),
		'boxed-logo'     => esc_attr__('Boxed view', 'optico'),
	);
	if( $for_vc==true ){ // for vc
		$return = array_flip( $return );
	}
...
function themestek_blogbox_description($limit=''){
	...
	if( has_excerpt() ){
		...
		$return  = '<p>'.do_shortcode($return).'</p>';
	...
	if( $ts_readmore_link=='yes' && !empty($return) && !empty($blog_readmore_text) ){
		$return .= themestek_wp_kses('<div class="themestek-box-desc-footer"><div class="themestek-blogbox-desc-footer"><div class="themestek-blogbox-footer-readmore"><div class="ts-read-more"><a href="'.get_permalink().'">'.$blog_readmore_text.'</a></div></div></div></div>');
	}
	if( 'link' == get_post_format() || 'quote' == get_post_format() ){
		$return = '';
	}
...
function themestek_blogbox_readmore_text(){
	$blog_readmore_text = themestek_get_option('blog_readmore_text');
	$return             = esc_attr__('Read More', 'optico');
...
function themestek_blogbox_readmore(){
	...
	if( strpos(get_the_content(), '"more-link"')!==false && get_post_format()!='quote' && get_post_format()!='link' ) {
		$return .= '<div class="themestek-blogbox-footer-left themestek-wrap-cell">';
...
function themestek_entry_icon() {
	...
	$iconCode .= '<i class="ts-optico-icon-'.$icon.'"></i>';
	// return data
	return $iconCode;
}
}
...
function themestek_box_wrapper( $position='start', $cptname='blog', $vars=array() ){
	...
	if( $position=='start' ){
		...
		if ( $boxview=='carousel' ) {
			if( $vars['carousel_nav']=='above' || $vars['carousel_nav']=='below' || $vars['carousel_nav']=='hide'  ){
				...
				$classArray[] = 'ts-boxes-carousel-arrows-side';
			}
		}
		// CSS Animation
...
function themestek_blogbox_single_meta( $option='date', $args=array() ) {
	...
	switch($option){
		...
		case 'category' :
			...
			$categories_list = get_the_category_list( _x( ', ', 'Used between list items, there is a space after the comma.', 'optico' ) );
			if ( !empty($categories_list) ) {
...
function themestek_titlebar_title(){
	...
	if( is_singular() || is_home() ){  // single page, single post and single cpt
		...
		if( is_home() ){
			...
			$title = esc_attr__( 'Blog', 'optico' );  // Setting for Titlebar title
		}
		...
		$adv_tbar_catarc = ( !empty($adv_tbar_catarc) ) ? esc_attr($adv_tbar_catarc.' %s') : esc_attr__('Category Archives: %s', 'optico') ;
		$title = sprintf(
			$adv_tbar_catarc,
		...
		$adv_tbar_tagarc = ( !empty( $adv_tbar_tagarc ) ) ? esc_attr($adv_tbar_tagarc.' %s') : esc_attr__('Tag Archives: %s','optico') ;
		$title = sprintf(
			$adv_tbar_tagarc,
		...
		if( is_tax('ts-team-group') || is_tax('ts-portfolio-category') ){
			...
			global $wp_query;
			$adv_tbar_postclassified = ( !empty($adv_tbar_postclassified) ) ? esc_attr($adv_tbar_postclassified.' %s') : esc_attr__('Posts classified under: %s', 'optico') ;
			$title = sprintf(
		...
		$adv_tbar_authorarc = ( !empty( $adv_tbar_authorarc ) ) ? esc_attr($adv_tbar_authorarc.' %s') : esc_attr__('Author Archives: %s', 'optico');
		$title = sprintf(
			$adv_tbar_authorarc,
		...
		$title    = sprintf( esc_attr__( 'Search Results for %s', 'optico' ), '<span class="ts-titlebar-heading ts-tbar-search-title">' . get_search_query() . '</span>' );
	} else if( is_404() ){ // 404
		...
		if( function_exists('tribe_is_month') && tribe_is_month() && !is_tax() ) { // The Main Calendar Page
			...
			global $wp_query;
			$title = esc_attr__( 'Events on: ', 'optico' ). date('F j, Y', strtotime($wp_query->query_vars['eventDate']));
		} elseif( function_exists('tribe_is_venue') && tribe_is_venue() ) { // Single Venues
			$title =	get_the_title();
		// BBPress section
		} else if( function_exists('is_bbpress') && is_bbpress() ) {
			$title = esc_attr__( 'Forum', 'optico' );
		} else if( is_post_type_archive() ){
			$title = post_type_archive_title('', false);
		} else if ( is_day() ){
			$title = sprintf( esc_attr__( 'Daily Archives: %s', 'optico' ), '<span>' . get_the_date() . '</span>' );
		} elseif( is_month() ){
			$title = sprintf( esc_attr__( 'Monthly Archives: %s', 'optico' ), '<span>' . get_the_date( _x( 'F Y', 'monthly archives date format', 'optico' ) ) . '</span>' );
		} elseif( is_year() ){
			$title = sprintf( esc_attr__( 'Yearly Archives: %s', 'optico' ), '<span>' . get_the_date( _x( 'Y', 'yearly archives date format', 'optico' ) ) . '</span>' );
		} else {
			if( function_exists('is_bbpress') && is_bbpress() ) {
				$title = esc_attr__( 'Forum', 'optico' );
			} else {
				$title = esc_attr__( 'Archives', 'optico' );
			}
		};
	} else {
		$title = get_the_title();
...
add_action( 'wp_enqueue_scripts', 'themestek_titlebar_inline_style', 18 );
/**
	...
	if( themestek_fbar_show()==true ){
		...
		<span class="themestek-fbar-btn ' . themestek_sanitize_html_classes(themestek_fbar_btn_classes()) . '">
			<a href="#" class="themestek-fbar-btn-link">
				...
				<span class="ts-hide">' . esc_attr__('Open', 'optico') . '</span>
			</a>
		</span>';
	}
...
function themestek_topbar_inline_style(){
	...
	if( $topbar_text_color=='custom' ){
		$return .= '.themestek-pre-header-wrapper, .themestek-pre-header-wrapper a{color:'. $topbar_text_custom_color .';}';
	}
	return $return;
}
}
...
function themestek_header_links(){
	...
	if( function_exists('is_woocommerce') && $wc_header_icon==true ){
		...
		$return  .= '<span class="ts-header-icon ts-header-wc-cart-link"><a href="' . $cart_url . '"><i class="ts-optico-icon-shopping-cart"></i><span class="number-cart">' . esc_attr($count) . '</span></a></span>';
	}
	// Search icon
	if( $header_search==true ){
		$class   = '';
		$return .= '<span class="ts-header-icon ts-header-search-link"><a href="#"><i class="ts-optico-icon-search-1"></i></a></span>';
	}
	if( $return!='' ){
...
function themestek_footer_cta(){
	...
	if( $footer_cta==true && shortcode_exists('ts-cta') && function_exists('themestek_url_encode') ){
		...
		$cta_sc = '[ts-cta h2="' . themestek_wp_kses($title) . '" h4="' . themestek_wp_kses($subtitle) . '" btn_title="' . esc_attr($btn_text) . '" btn_link="url:' . themestek_url_encode($btn_link) . '|||" add_icon="left" i_type="' . $i_library . '" ' . $i_icon . ' shape="square" btn_shape="rounded" btn_color="black" btn_font_weight="yes" btn_align="right" i_color="black" i_background_style="rounded" i_background_color="white" i_size="lg"][/ts-cta]';
		//$cta_sc = '[ts-cta h2="If you Have Any Questions Schedule an Appointment<strong>With Our Doctor OR Call Us On (010)123-456-7890</strong>" h2_font_container="font_size:22px|line_height:36px" h4_font_container="font_size:20px" shape="square" btn_title=" Make an Appointment" btn_shape="rounded" btn_color="black" btn_font_weight="yes" btn_align="right" add_icon="left" i_type="themify" i_icon_themify="themifyicon ti-headphone-alt" i_color="black" i_background_style="rounded" i_background_color="white" i_size="lg" use_custom_fonts_h2="true" use_custom_fonts_h4="true"][/ts-cta]';
...
function themestek_footer_row_class( $row='first' ){
	...
	global $optico_theme_options;
	// BG color
	if( !empty($optico_theme_options[$row.'_footer_bg_color']) ){
		$class .= ' ts-bg ts-bgcolor-'.sanitize_html_class($optico_theme_options[$row.'_footer_bg_color']);
	}
	// Text color
	if( !empty($optico_theme_options[$row.'_footer_text_color']) ){
		$class .= ' ts-textcolor-'.sanitize_html_class($optico_theme_options[$row.'_footer_text_color']);
	}
	// If bg image is there
	if( !empty($optico_theme_options[$row.'_footer_bg_all']['image']) ){
		$class .= ' ts-bgimage-yes';
	} else {
		$class .= ' ts-bgimage-no';
	}
	// border type
	if( !empty($optico_theme_options[$row.'_footer_bordertype']) ){
		$class .= ' ts-bordertype-'.sanitize_html_class($optico_theme_options[$row.'_footer_bordertype']);
	}
	//widget border color
	if( !empty($optico_theme_options[$row.'_footer_widget_bordercolor']) ){
		$class .= ' ts-widgetbordercolor-'.sanitize_html_class($optico_theme_options[$row.'_footer_widget_bordercolor']);
	}
			...
			$return .= '
				<li>
					<a href="'. $social['social_icons_list_link'] .'" target="_blank" class="'.$a_color_class.'">
						<i class="ts-optico-icon-'. $social['social_icons_list_icon'] .'"></i>
 ...
 */
if( !function_exists('optico_addhttp') ){
	function optico_addhttp($url){
		if (!preg_match("~^(?:f|ht)tps?://~i", $url)){
			$url = "http://" . $url;
		}
		return $url;
...
function themestek_testimonial_star_ratings(){
	...
	for ($i = 1; $i <= 5; $i++) {
		if( $star_ratings >= $i ){
			$return .= ' <i class="ts-optico-icon-star ts-skincolor ts-active"></i> ';
		} else {
			$return .= ' <i class="ts-optico-icon-star"></i> ';
		}
	}
	return $return;
}
...
function themestek_testimonial_featured_image($size='thumbnail'){
	...
	$return = ( !empty($featured_image) ) ? $featured_image : '<span class="themestek-icon-box"><i class="demo-icon ts-optico-icon-quote-left"></i></span>';
	return $return;
}
}
/**
...
function themestek_header_button( $args=array() ){
	...
	if( !empty($btn_text) && !empty($btn_link) && shortcode_exists('ts-btn') && function_exists('themestek_url_encode') ){
		$btn_sc		= '[ts-btn title="' . esc_attr($btn_text) . '" shape="rounded" style="outline" color="skincolor" font_weight="yes" size="md" link="url:' . themestek_url_encode($btn_link) . '|||"]';
		$headerstyle = themestek_get_option('headerstyle');
		if( $headerstyle=='classic-overlay' ){
			$btn_sc		= '[ts-btn title="' . esc_attr($btn_text) . '" shape="rounded" color="white" font_weight="yes" link="url:' . themestek_url_encode($btn_link) . '|||"]';
		}
 ...
 *
 */
if( !function_exists('themestek_header_text') ){
function themestek_header_text(){
	$return		 = '';
	$headerstyle = themestek_get_headerstyle();
	$header_text = themestek_get_option('header_text');
	...
	if( in_array( $headerstyle, $valid_headerstyle ) && !empty( $header_text ) ){
		$header_text = themestek_wp_kses( $header_text );
		$return      = '<div class="ts-header-text-area">'.do_shortcode( $header_text ).'</div>';
	}
	echo  themestek_wp_kses($return);
}
}
...
function themestek_revslider_array( $countonly=false ) {
	...
	if( $countonly==true ){
		...
		if( count($sliders)==0 ){
			$sliders[''] = esc_attr__('(No Slider Found)', 'optico');
		}
		return $sliders;
	}
}
	...
	if ( function_exists('lsSliders') ) {
		...
		if( $countonly == true ){
			...
			if( count($slider_names) == 0 ){
				$slider_names[''] = esc_attr__('(No Slider Found)', 'optico');
			}
			return $slider_names;
		}
	}
...
function themestek_single_get_sidebar_value(){
	...
	$sidebar = $optico_theme_options['sidebar_post'];
	if( is_page() || is_singular() ){
		...
		if( !empty($optico_theme_options['sidebar_'.$cpt]) ){
			$sidebar = $optico_theme_options['sidebar_'.$cpt];
		}
	...
	if( is_search() ){
		$sidebar = $optico_theme_options['sidebar_search'];
	}
	// If search results page
	if( is_search() ){
		$sidebar = $optico_theme_options['sidebar_search'];
...
function themestek_event_description(){ 
	$return   = '';
	$readMore = esc_attr__('See Event', 'optico') . ' <i class="tsicon-fa-angle-right"></i>';
	if( has_excerpt() ){
...
function themestek_comment_row_template($comment, $args, $depth){
    ...
    <<?php echo esc_attr($tag); ?> <?php comment_class( empty( $args['has_children'] ) ? '' : 'parent' ) ?> id="comment-<?php comment_ID() ?>">
		...
		<?php if ( $comment->comment_approved == '0' ) : ?>
			 <em class="comment-awaiting-moderation"><?php esc_attr_e( 'Your comment is awaiting moderation.', 'optico' ); ?></em>
			  <br />
		<?php endif; ?>
		<div class="comment-meta commentsetadata">
			<?php printf( '<cite class="ts-comment-owner fn">%s</cite> <span class="says">' . esc_attr__('says:', 'optico' ) . '</span>', get_comment_author_link() ); ?>
			<a href="<?php echo htmlspecialchars( get_comment_link( $comment->comment_ID ) ); ?>">
			<?php
			/* translators: 1: date, 2: time */
			printf( esc_attr__( '%1$s at %2$s', 'optico' ), get_comment_date(),  get_comment_time() ); ?></a><?php edit_comment_link( esc_attr__( '(Edit)', 'optico' ), '  ', '' );
			?>
		</div>
		<?php comment_text(); ?>
		<div class="reply">
...
function themestek_author_social_links(){
	...
	foreach( $all_socials as $social_class => $social_link ){
		if( !empty($social_link) ){
			$return .= '<li><a href="'. $social_link .'" target="_blank"><i class="ts-optico-icon-'. $social_class .'"></i><span class="ts-hide">'. ucwords($social_class) .'</span></a></li>';
		}
	}
	if( !empty($return) ){
...
function themestek_search_results_box_title( $post_type='post' ){
	$return        = '';
	$singular_name = '';
	$small_link = '<small><a href="'. esc_url(get_home_url()).'?s='.get_search_query().'" class="label label-default"><i class="ts-optico-icon-angle-left"></i> '.esc_attr__('Back to results','optico').'</a></small>';
	if( empty($_GET['post_type']) ){
		$small_link = '<small><a href="'. esc_url(get_home_url()).'?s='.get_search_query().'&post_type=' . $post_type . '" class="label label-default">'.esc_attr__('View more','optico').'</a></small>';
	}
	if( !empty($post_type) ){
	...
	if( !empty($cpt_obj) ){
		foreach( $cpt_obj as $cpt ){
			if( !in_array( $cpt->name, array( 'attachment', 'ts-client', 'ts-testimonial' ) ) ){  // Exclude Media from search
				...
				$results_text  .= ( $total_results > 0 ) ? '  <li><a href="' . esc_url($link) . '"> <i class="ts-optico-icon-angle-right"></i> ' . sprintf( esc_attr__('%1$s found in %2$s','optico'), $total_results, '<strong>'.$cpt->labels->name.'</strong>' ) . '</a></li>' : '' ;
			}
		}
	}
	if( !empty($results_text) ){
		$return = '<div class="ts-search-no-results ts-search-results-others"><p>' . sprintf( esc_attr__('Sorry, but nothing matched in %1$s with your search terms.  But we found results in other sections:','optico'), '<strong>' . $current_cpt_name . '</strong>' ) . '</p>';
		...
		$return = '<p>'.esc_attr__('Sorry, but nothing matched your search terms. Please try again with some different keywords.','optico').'</p>';
	}
	echo themestek_wp_kses($return);
}
}
...
function themestek_search_form(){
	...
	if( !empty($cpt_obj) ){
		foreach( $cpt_obj as $cpt ){
			if( !in_array( $cpt->name, array( 'attachment', 'ts-client', 'ts-testimonial' ) ) ){  // Exclude Media from search
				...
				$total_results = $allsearch->found_posts;
				if( $total_results > 0 ){
					$results_text  = '  <span>'.sprintf( esc_attr__('%1$s found','optico'), $total_results ).'</span>';
	...
	<div class="ts-search-form-wrapper <?php echo esc_attr($no_tab_class); ?>">
		<div class="ts-search-form-big-icon"><i class="ts-optico-icon-search-1"></i></div>
		<form role="search" method="get" class="search-form ts-search-form" action="<?php echo esc_url( home_url( '/' ) ); ?>">
			<label class="ts-searchword">
				<span class="screen-reader-text"><?php echo _x( 'Search for:', 'label', 'optico' ); ?></span>
				<input type="search" class="search-field" placeholder="<?php echo esc_attr_x( 'Search &hellip;', 'placeholder', 'optico' ); ?>" value="<?php echo get_search_query(); ?>" name="s" />
			</label>
			<input type="hidden" name="post_type" value="<?php echo esc_attr( $current_cpt ); ?>" />
			<input type="submit" class="search-submit" value="<?php echo esc_attr_x( 'Search', 'submit button', 'optico' ); ?>" />
		</form>
...
function themestek_entry_date( $echo = true ) {
	if ( has_post_format( array( 'chat', 'status' ) ) ){
		$format_prefix = _x( '%1$s on %2$s', '1: post format name. 2: date', 'optico' );
	} else {
		$format_prefix = '%2$s';
	}
...
if ( ! function_exists( 'optico_blogstyle2_meta' ) ) :
function optico_blogstyle2_meta( $metafor="blogbox" ) {
	if( !in_array($metafor, array('blogclassic','blogbox') ) ){
		$metafor = "blogclassic";
	}
	$return       = '';
...
if ( ! function_exists( 'optico_posttime_meta' ) ) :
function optico_posttime_meta( $metafor="blogbox" ) {
	if( !in_array($metafor, array('blogclassic','blogbox') ) ){
		$metafor = "blogclassic";
	}
	$return       = '';
...
function themestek_get_socialbar_links( $tooltip_position='top' , $tooltip='yes' ){
	...
	$return = '';
	if( !empty($optico_theme_options['social_icons_list']) ){
		foreach( $optico_theme_options['social_icons_list'] as $socialicon ){
			...
			$return .= '<li class="ts-social-' . $social_id . ' ts-socialbox-i-wrapper"><a class="ts-socialbox-icon-link ts-socialbox-icon-link-' . $social_id . ' ' . sanitize_html_class($class) . '" target="_blank" href="' . $href . '"><span class="frame-hover"></span><i class="ts-optico-icon-' . $social_id . '"></i><span class="social_name">'.$social_name.'</span></a></li>' . "\n";
		}
	}
	foreach( $socialArray as $key=>$value ){
		if( $key == 'rss' ){
			if( isset($optico_theme_options['rss']) && $optico_theme_options['rss']=='1' ){
			...
			if( isset($optico_theme_options[$key]) && trim($optico_theme_options[$key])!='' ){
				$return .= '<li class="'.$key.' "><a target="_blank" href="'.esc_url($optico_theme_options[$key]).'" data-tooltip="'.$value[1].'"><i class="ts-social-icon-'.$value[0].'"></i></a></li>';
			}
		}
	}
	if( $return!='' ){
