...
$params = array_merge(
	...
	array(
		...
		array(
			'type'        => 'dropdown',
			'heading'     => esc_attr__( 'Seperator', 'optico' ),
			'param_name'  => 'seperator',
			'value'       => array(
				esc_attr__( 'None', 'optico' )  							=> 'none',
				esc_attr__( 'Solid', 'optico' ) 							=> 'solid',
			),
			'description' => esc_attr__( 'Show separator between Heading and Subheading', 'optico' ),
			'std'		  => 'none',
		),
...
foreach( $params as $key=>$val ){
	...
	if( !empty($val['param_name']) && $val['param_name']=='h2_link' ){
		...
		$params[$key]['heading'] = esc_attr__('Heading Link (URL)','optico');
	}
	...
	if( !empty($val['param_name']) && $val['param_name']=='h4_link' ){
		unset($params[$key]['group']);
		unset($params[$key]['dependency']);
		$params[$key]['heading'] = esc_attr__('Subheading Link (URL)','optico');
	}
}
...
vc_map( array(
	'name'     => esc_attr__( 'ThemeStek Heading and Subheading', 'optico' ),
	'base'     => 'ts-heading',
